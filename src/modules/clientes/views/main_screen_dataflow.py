...
class MainScreenDataflowMixin:
    ...
    def _sort_by(self, column: str) -> None:
        ...
        if column == "updated_at":
            ...
            self.var_ordem.set(mapping[column])  # pyright: ignore[reportAttributeAccessIssue]
        elif column == "id":
    ...
    def _buscar(self, _event: Any | None = None) -> None:
        try:
            if self._buscar_after:  # pyright: ignore[reportAttributeAccessIssue]
                self.after_cancel(self._buscar_after)  # pyright: ignore[reportAttributeAccessIssue]
        except Exception as exc:  # noqa: BLE001
            log.debug("Falha ao cancelar debounce de busca: %s", exc)

        self._buscar_after = self.after(200, self.carregar)  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _populate_status_filter_options(self) -> None:
        statuses = self._vm.get_status_choices()  # pyright: ignore[reportAttributeAccessIssue]

        choices = build_filter_choices_with_all_option(statuses)

        try:
            self.status_filter.configure(values=choices)  # pyright: ignore[reportAttributeAccessIssue]

        except Exception as exc:  # noqa: BLE001
            log.debug("Falha ao atualizar filtro de status: %s", exc)

        current = (self.var_status.get() or "").strip()  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _get_clients_for_controller(self) -> Sequence[ClienteRow]:
        ...
        raw_clientes = self._vm._clientes_raw  # pyright: ignore[reportPrivateUsage,reportAttributeAccessIssue]

        # Converter cada cliente raw para ClienteRow usando o método do VM
        rows: list[ClienteRow] = []
        for cliente in raw_clientes:
            row = self._vm._build_row_from_cliente(cliente)  # pyright: ignore[reportPrivateUsage,reportAttributeAccessIssue]
            rows.append(row)
    ...
    def _update_ui_from_computed(self, computed: MainScreenComputedLike) -> None:
        ...
        self._current_rows = list(computed.visible_clients)  # pyright: ignore[reportAttributeAccessIssue]
        self._render_clientes(self._current_rows)  # pyright: ignore[reportAttributeAccessIssue]

        # Atualizar SelectionManager com novos clientes
        self._selection_manager.update_all_clients(self._current_rows)  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _update_batch_buttons_from_computed(self, computed: MainScreenComputedLike) -> None:
        ...
        try:
            if getattr(self, "btn_batch_delete", None) is not None:
                self.btn_batch_delete.configure(state="normal" if computed.can_batch_delete else "disabled")  # pyright: ignore[reportAttributeAccessIssue]

            if getattr(self, "btn_batch_restore", None) is not None:
                self.btn_batch_restore.configure(state="normal" if computed.can_batch_restore else "disabled")  # pyright: ignore[reportAttributeAccessIssue]

            if getattr(self, "btn_batch_export", None) is not None:
                self.btn_batch_export.configure(state="normal" if computed.can_batch_export else "disabled")  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _apply_status_for(self, cliente_id: int, chosen: str) -> None:
        ...
        if decision.kind == "error":
            if decision.message:
                messagebox.showerror("Erro", decision.message, parent=self)  # pyright: ignore[reportArgumentType]
            return
        ...
        if decision.kind == "execute":
            ...
            try:
                ...
                _ = self._vm.extract_status_and_observacoes(old_obs)  # pyright: ignore[reportAttributeAccessIssue]
                ...
                messagebox.showerror("Erro", f"Falha ao atualizar status: {e}", parent=self)  # pyright: ignore[reportArgumentType]
    ...
    def _apply_connectivity_state(self, state: str, description: str, text: str, _style: str, _tooltip: str) -> None:
        ...
        raw = ConnectivityRawInput(
            ...
            text=text,
            last_known_state=self._last_cloud_state if hasattr(self, "_last_cloud_state") else "unknown",  # pyright: ignore[reportArgumentType,reportAttributeAccessIssue]
        )

        # Computar snapshot de conectividade
        snapshot = self._connectivity_state_manager.compute_snapshot(raw)  # pyright: ignore[reportAttributeAccessIssue]

        # Aplicar atributos globais da app
        try:
            if self.app is not None:  # pyright: ignore[reportAttributeAccessIssue]
                setattr(self.app, "_net_is_online", snapshot.is_online)  # pyright: ignore[reportAttributeAccessIssue]
                setattr(self.app, "_net_state", snapshot.state)  # pyright: ignore[reportAttributeAccessIssue]
                setattr(self.app, "_net_description", snapshot.description)  # pyright: ignore[reportAttributeAccessIssue]
        except Exception as exc:  # noqa: BLE001
        ...
        status_var = getattr(self.app, "status_var_text", None) if self.app is not None else None  # pyright: ignore[reportAttributeAccessIssue]
        ...
        if snapshot.should_log_transition:
            ...
            self._last_cloud_state = snapshot.state  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _update_main_buttons_state(self, *_: Any) -> None:
        ...
        try:
            ...
            if not pick_snapshot.is_pick_mode_active:
                self.btn_lixeira.configure(state=("normal" if button_states.lixeira else "disabled"))  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _set_count_text(self, count: int, clientes: Sequence[Any] | None = None) -> None:
        ...
        try:
            ...
            summary = compute_count_summary(
                visible_clients=self._current_rows if hasattr(self, "_current_rows") else [],  # pyright: ignore[reportAttributeAccessIssue]
                raw_clients_for_stats=clientes,
            )

            # Atualizar o StatusFooter global
            if self.app and self.app.status_footer:  # pyright: ignore[reportAttributeAccessIssue]
