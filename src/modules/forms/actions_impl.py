"""
Implementação de ações de formulários (migrado de src.ui.forms.actions).

Módulo oficial para lógica de:
- Preenchimento via Cartão CNPJ
- Upload de documentos para Supabase
- Download e listagem de arquivos do Storage
- Dialogs de subpastas (SubpastaDialog)

Este módulo contém a implementação real. Use src.modules.forms.actions como entrypoint público.
"""
...
class EntryLike(Protocol):
    ...
    def insert(self, index: int | str, _string: str) -> None: ...
    def get(self) -> str: ...
