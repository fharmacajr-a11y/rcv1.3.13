...
def check_duplicates(
    *,
    ...
    """
    Checa duplicatas por CNPJ / Razão Social.
    (WhatsApp/numero foi liberado no app: não bloqueia mais.)
    ...
    Retorna:
      {
        'duplicates': { 'CNPJ': [ids...], 'RAZAO_SOCIAL': [ids...] },
        'has_any': bool
      }
    """
    dup: Dict[str, list] = {"CNPJ": [], "RAZAO_SOCIAL": []}

    cnpj_d = normalize_cnpj(cnpj)
    # numero_d = only_digits(numero)  # não usamos mais pra bloquear
