...
class CTkTreeview(ttk.Treeview):
    def __init__(
        ...
        **kw,
        ...
        # Scrollbar
        self.scrollbar = ctk.CTkScrollbar(self.frame, orientation="vertical", command=self.yview)
    ...
    def configure(self, require_redraw=False, **kw):
        ...
        if "selectmode" in kw:
            self.selectmode = cast("Literal['browse', 'extended', 'none']", kw.pop("selectmode"))
            options["selectmode"] = self.selectmode

        if "show" in kw:
            self.show = cast("Literal['tree', 'headings', 'tree headings', ''] | Iterable[str]", kw.pop("show"))
