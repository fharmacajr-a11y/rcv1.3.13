...
class CTkSplitPane(ctk.CTkFrame):
    ...
    def _create_sash(self) -> None:
        ...
        if self._orient == "horizontal":
            self._sash = ctk.CTkFrame(self, width=self._sash_width, fg_color=self._sash_color, cursor="sb_h_double_arrow")
            self._sash.grid(row=0, column=1, sticky="ns")
        else:
            self._sash = ctk.CTkFrame(self, height=self._sash_width, fg_color=self._sash_color, cursor="sb_v_double_arrow")
