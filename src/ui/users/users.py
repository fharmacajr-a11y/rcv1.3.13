...
class UserManagerDialog(ctk.CTkToplevel):
    def __init__(self, master=None, current_role="admin"):
        ...
        if is_admin:
            make_btn(
                footer, text="Adicionar", command=self.add_user, fg_color=BTN_SUCCESS, hover_color=BTN_SUCCESS_HOVER
            ).pack(side="left", padx=5)
            make_btn(
                footer, text="Editar", command=self.edit_user, fg_color=BTN_PRIMARY, hover_color=BTN_PRIMARY_HOVER
            ).pack(side="left", padx=5)
            make_btn(
                footer, text="Excluir", command=self.delete_user, fg_color=BTN_DANGER, hover_color=BTN_DANGER_HOVER
            ).pack(side="left", padx=5)

        # Direita: ↻ (ícone) e Fechar
        make_btn(
            footer, text="Fechar", command=self.destroy, fg_color=BTN_SECONDARY, hover_color=BTN_SECONDARY_HOVER
        ).pack(side="right", padx=5)
        make_btn(
            footer, text="↻", command=self.load_users, width=40, fg_color=BTN_SECONDARY, hover_color=BTN_SECONDARY_HOVER
        ).pack(side="right", padx=5)
    ...
    def _user_form(self, title: str, user_id=None, username: str = ""):
        ...
        # Usuário
        ctk.CTkLabel(frm, text="Usuário").grid(row=0, column=0, sticky="w")
        ent_user = ctk.CTkEntry(frm, width=280)
        ...
        # Senha + confirmar
        ctk.CTkLabel(frm, text="Senha").grid(row=2, column=0, sticky="w")
        ent_pwd = ctk.CTkEntry(frm, show="*", width=280)
        ent_pwd.grid(row=3, column=0, sticky="ew", pady=(0, 8))

        ctk.CTkLabel(frm, text="Confirmar senha").grid(row=4, column=0, sticky="w")
        ent_pwd2 = ctk.CTkEntry(frm, show="*", width=280)
        ...
        make_btn(btns, text="Cancelar", command=_close, fg_color=BTN_SECONDARY, hover_color=BTN_SECONDARY_HOVER).pack(
            side="right", padx=4
        )
        make_btn(btns, text="Salvar", command=_save, fg_color=BTN_SUCCESS, hover_color=BTN_SUCCESS_HOVER).pack(
            side="right"
        )
