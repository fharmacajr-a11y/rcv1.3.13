...
def create_search_controls(
    ...
    *,
    ...
    if search_icon is not None:
        try:
            icon_label = tk.Label(search_container, image=search_icon, bg=frame_bg, borderwidth=0)
    ...
    if search_var is not None:
        _trace_ids: list[tuple[tk.StringVar, str]] = []
        _tid = search_var.trace_add("write", lambda *_args: _update_placeholder())
        _trace_ids.append((search_var, _tid))
    else:
        _trace_ids = []
    _update_placeholder()
    ...
    if order_var is not None:
        _tid = order_var.trace_add("write", lambda *_args: _clear_combobox_selection(order_combobox))
        _trace_ids.append((order_var, _tid))
    _clear_combobox_selection(order_combobox)
    ...
    if status_var is not None:
        _tid = status_var.trace_add("write", lambda *_args: _clear_combobox_selection(status_combobox))
        _trace_ids.append((status_var, _tid))
    _clear_combobox_selection(status_combobox)
    ...
    # --- Cleanup de traces ao destruir o frame ---
    def _cleanup_traces() -> None:
        for var, tid in _trace_ids:
            try:
    ...
    frame.bind("<Destroy>", lambda _e: _cleanup_traces(), add="+")
