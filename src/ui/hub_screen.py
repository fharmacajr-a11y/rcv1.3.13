...
from src.ui.hub.constants import PAD_OUTER, MODULES_TITLE
...
from src.ui.hub.actions import (
    on_add_note_clicked as actions_on_add_note_clicked,
    on_show as actions_on_show,
)
from src.ui.hub.state import HubState, ensure_hub_state as ensure_state
from src.ui.hub.controller import (
    append_note_incremental as controller_append_note_incremental,
    cancel_poll as controller_cancel_poll,
    on_realtime_note as controller_on_realtime_note,
    poll_notes_if_needed as controller_poll_notes_if_needed,
    refresh_notes_async as controller_refresh_notes_async,
    retry_after_table_missing as controller_retry_after_table_missing,
    schedule_poll as controller_schedule_poll,
)
...
class HubScreen(tb.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        open_clientes = (
            open_clientes or kwargs.pop("on_open_clientes", None) 
            or open_sifap or kwargs.pop("on_open_sifap", None)
        )
        ...
        def mk_btn(text: str, cmd: Optional[Callable] = None, highlight: bool = False, yellow: bool = False) -> tb.Button:
            ...
            if yellow:
                b = tb.Button(modules_panel, text=text, command=(cmd or self._noop), bootstyle="warning")
            elif highlight:
    ...
    def _refresh_author_names_cache_async(self, force: bool = False) -> None:
        ...
        def _work():
            ...
            prefix_map = {}
            try:
                from src.core.services.profiles_service import get_display_names_map, get_email_prefix_map
    ...
    def _collect_notes_debug(self) -> dict:
        ...
        notes = (
            ...
            or getattr(self, "_notes_last_snapshot", None) or []
        )
        out = {
            "org_id": self._get_org_id_safe() if hasattr(self, "_get_org_id_safe") else None,
            "current_user": (
                getattr(self, "_current_user_email", None) 
                or self._get_email_safe() if hasattr(self, "_get_email_safe") else ""
            ),
