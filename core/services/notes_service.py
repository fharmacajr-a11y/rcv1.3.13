...
def _check_table_missing_error(exception: Exception) -> None:
    ...
    if "pgrst205" in error_str or "relation" in error_str and "does not exist" in error_str:
        raise NotesTableMissingError(
            f"Tabela '{TABLE}' ausente no Supabase. Execute a migration: infra/db/rc_notes_migration.sql"
        )
...
def _normalize_author_emails(rows: List[dict], org_id: str) -> List[dict]:
    ...
    """
    try:
        from core.services.profiles_service import EMAIL_PREFIX_ALIASES, get_email_prefix_map
...
def add_note(org_id: str, author_email: str, body: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            log.warning("Erro de permissão RLS (42501): org_id=%s, author=%s", org_id, email_prefix)
            raise
