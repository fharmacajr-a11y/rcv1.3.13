...
def _ask_subpasta_nome(parent: Any, default: str = "") -> Optional[str]:
...
def perform_uploads(*args, **kwargs) -> Tuple[tuple, Dict[str, Any]]:
    ...
    from ui.forms import actions as actions_module
...
def finalize_state(*args, ctx_override: Optional[UploadCtx] = None, **kwargs) -> Tuple[tuple, Dict[str, Any]]:
    ...
    msg = (
        ...
        if ctx.falhas == 0
        else f"Cliente salvo com {ctx.falhas} falha(s)."
    )
    try:
