# 📚 Documentação RC Gestor

Índice completo da documentação organizada do projeto.

---

## 🔧 Patches e Correções

Documentação de patches aplicados durante o desenvolvimento:

| Arquivo | Descrição |
|---------|-----------|
| [PATCH_V2_DOUBLECLICK_DETERMINISTICO.md](patches/PATCH_V2_DOUBLECLICK_DETERMINISTICO.md) | Correção do double-click determinístico na v2 |
| [PATCH_CLIENTESV2_DOUBLECLICK_FLASH.md](patches/PATCH_CLIENTESV2_DOUBLECLICK_FLASH.md) | Patch para flash no double-click |
| [PATCH_CLIENT_FILES_BROWSER.md](patches/PATCH_CLIENT_FILES_BROWSER.md) | Correções no navegador de arquivos |
| [PATCH_FIX_FILES_BROWSER_ACCESS.md](patches/PATCH_FIX_FILES_BROWSER_ACCESS.md) | Fix de acesso ao browser de arquivos |
| [ANALISE_MIGRACAO_CTK_CLIENTESV2.md](patches/ANALISE_MIGRACAO_CTK_CLIENTESV2.md) | Análise da migração CTK no módulo clientes v2 |

---

## 📊 Relatórios

### Microfases

| Arquivo | Descrição |
|---------|-----------|
| [RELATORIO_MICROFASE_35.md](reports/microfases/RELATORIO_MICROFASE_35.md) | Relatório da microfase 35 |
| [MICROFASE_36_RELATORIO_FINAL.md](reports/microfases/MICROFASE_36_RELATORIO_FINAL.md) | Relatório final da microfase 36 |
| [RELATORIO_MICROFASE_37.md](reports/microfases/RELATORIO_MICROFASE_37.md) | Relatório da microfase 37 |
| [RELATORIO_MIGRACAO_CTK_COMPLETA.md](reports/microfases/RELATORIO_MIGRACAO_CTK_COMPLETA.md) | Relatório completo da migração CTK |

### Releases

| Arquivo | Descrição |
|---------|-----------|
| [EXECUTIVE_SUMMARY.md](reports/releases/EXECUTIVE_SUMMARY.md) | Resumo executivo |
| [GATE_FINAL.md](reports/releases/GATE_FINAL.md) | Gate final do release |
| [CI_GREEN_REPORT.md](reports/releases/CI_GREEN_REPORT.md) | Relatório de CI verde |
| [NEXT_STEPS.md](reports/releases/NEXT_STEPS.md) | Próximos passos |
| [PR_DESCRIPTION.md](reports/releases/PR_DESCRIPTION.md) | Descrição de pull request |

---

## 📖 Guias

| Arquivo | Descrição |
|---------|-----------|
| [MIGRACAO_CTK_GUIA_COMPLETO.ipynb](guides/MIGRACAO_CTK_GUIA_COMPLETO.ipynb) | Guia completo de migração CustomTkinter (Jupyter Notebook) |

---

## 🏗️ Arquitetura e Desenvolvimento

Para informações sobre arquitetura, configuração e contribuição, consulte:

- [README principal (raiz)](../README.md) - Visão geral do projeto
- [CONTRIBUTING.md](../CONTRIBUTING.md) - Guia de contribuição
- [CHANGELOG.md](../CHANGELOG.md) - Histórico de mudanças

---

## 📋 Documentação Principal

### Status e Roadmap

- [STATUS.md](STATUS.md) - Estado atual do projeto e próximos passos
- [ROADMAP.md](ROADMAP.md) - Histórico completo de fases e decisões

### CI/CD

- [ci/README.md](ci/README.md) - Visão geral de CI/CD
- [ci/REFERENCE.md](ci/REFERENCE.md) - Quick reference para desenvolvimento
- [ci/STAGING_CHECKLIST.md](ci/STAGING_CHECKLIST.md) - Roteiro de smoke test

### Releases

- [releases/RELEASE_NOTES.md](releases/RELEASE_NOTES.md) - Notas de releases consolidadas
- [releases/TEMPLATES.md](releases/TEMPLATES.md) - Templates de PR e Release

### Migração CustomTkinter

- [customtk/README.md](customtk/README.md) - Índice da migração CTK
- [customtk/MIGRATION_SUMMARY.md](customtk/MIGRATION_SUMMARY.md) - Resumo completo (53 microfases)
- [customtk/TECHNICAL_DOCS.md](customtk/TECHNICAL_DOCS.md) - Políticas e configurações técnicas

### Cronologia

Documentos organizados por data disponíveis em: [cronologia/](cronologia/)

---

*Última atualização: 26 de janeiro de 2026*
