...
def test_fluxo_salvar_cliente_com_upload_integra_pipeline_e_service(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(prepare_module, "get_supabase_state", lambda: ("online", "Conectado ao Supabase e dentro do limiar de estabilidade."))

    # Mock funções em _prepare
    import src.modules.clientes.forms._prepare as prepare_module

    monkeypatch.setattr(prepare_module, "salvar_cliente", lambda row, valores: (123, str(tmp_path / "cliente123")))
...
def test_fluxo_lixeira_cliente_move_lista_restaura(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(db_manager_module, "get_cliente_by_id", lambda cid: SimpleNamespace(id=cid, razao_social=f"Cliente {cid}", numero="001"))
