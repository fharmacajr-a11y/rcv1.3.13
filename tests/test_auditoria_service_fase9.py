...
def test_get_current_org_id_lookup_error(mock_supabase):
    ...
    with (
        patch("src.modules.auditoria.service.get_supabase", return_value=mock_supabase),
        patch("src.modules.auditoria.service.repository.fetch_current_user_id", side_effect=LookupError("User not found")),
    ):
...
def test_ensure_auditoria_folder_success(mock_supabase):
    ...
    with (
        ...
        mock_ctx.return_value = AuditoriaStorageContext(
            bucket="clients", org_id="org-123", client_root="org-123/456", auditoria_prefix="org-123/456/GERAL/Auditoria"
        )
...
def test_ensure_auditoria_folder_custom_org_id(mock_supabase):
    ...
    with (
        ...
        mock_ctx.return_value = AuditoriaStorageContext(
            bucket="clients", org_id="custom-org", client_root="custom-org/789", auditoria_prefix="custom-org/789/GERAL/Auditoria"
        )
...
def test_list_existing_file_names_success(mock_supabase):
    ...
    with (
        patch("src.modules.auditoria.service.get_supabase", return_value=mock_supabase),
        patch("src.modules.auditoria.service.storage.list_existing_file_names", return_value={"file1.pdf", "file2.zip"}),
    ):
...
def test_upload_storage_bytes_success(mock_supabase):
    ...
    with (
        ...
        upload_storage_bytes(bucket="clients", dest_path="org/file.pdf", data=b"PDF content", content_type="application/pdf")
...
def test_upload_storage_bytes_with_upsert(mock_supabase):
    ...
    with (
        ...
        upload_storage_bytes(bucket="clients", dest_path="org/file.pdf", data=b"content", content_type="application/pdf", upsert=True)
...
def test_ensure_storage_ready_success():
    ...
    with patch("src.modules.auditoria.service.is_online", return_value=True), patch("src.modules.auditoria.service.get_clients_bucket", return_value="clients"):
...
def test_ensure_storage_ready_no_bucket():
    ...
    with patch("src.modules.auditoria.service.is_online", return_value=True), patch("src.modules.auditoria.service.get_clients_bucket", return_value=""):
...
def test_get_storage_context_uses_current_org_id():
    ...
    with (
        ...
        mock_make.return_value = AuditoriaStorageContext(
            bucket="clients", org_id="auto-org", client_root="auto-org/100", auditoria_prefix="auto-org/100/GERAL/Auditoria"
        )
