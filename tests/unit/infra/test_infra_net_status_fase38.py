...
def test_can_resolve_success():
    ...
    from src.infra.net_status import _can_resolve
...
def test_ok_code_204():
    ...
    from src.infra.net_status import _ok
...
def test_pick_base_with_url_hint_https():
    ...
    from src.infra.net_status import _pick_base
...
def test_pick_base_from_supabase_client_supabase_url():
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch.dict("sys.modules", {"src.infra.supabase_client": MagicMock(supabase=mock_supabase)}):
            result = _pick_base(None)
...
def test_pick_base_fallback_google():
    ...
    with patch.dict(os.environ, {}, clear=True):
        ...
        with patch.dict(sys.modules, {"src.infra.supabase_client": fake_module}):
...
def test_pick_base_supabase_client_exception():
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch.dict("sys.modules", {"src.infra.supabase_client": None}):
            result = _pick_base(None)
...
def test_probe_host_parsing_exception_continues():
    ...
    with patch("src.infra.net_status._pick_base", side_effect=mock_pick_base):
        with patch("httpx.Client", return_value=mock_client):
            result = probe("test")
