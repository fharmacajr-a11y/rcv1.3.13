...
def is_comment_or_docstring(line: str) -> bool:
    ...
    if stripped and stripped[0].isupper() and (stripped.endswith('.') or stripped.endswith(':')) and not stripped.startswith("class") and not stripped.startswith("def"):
...
def check_ttk_style_without_master(files: list[Path], src_dir: Path) -> list[Violation]:
    ...
    allowed_file = src_dir / "ui" / "ttk_compat.py"
    
    for file in files:
        if file == allowed_file:
            continue  # ttk_compat.py tem docstrings que mencionam ttk.Style(), permitir
            ...
            with open(file, "r", encoding="utf-8") as f:
                ...
                for line_no, line in enumerate(lines, start=1):
                    ...
                    if pattern.search(line):
                        ...
                        file_str = str(file)
                        if "utils/themes.py" in file_str or "helpers/hidpi.py" in file_str or "theme_setup.py" in file_str:
                            continue
...
def main() -> int:
    ...
    print("🔍 Validando política UI/Theme...")
