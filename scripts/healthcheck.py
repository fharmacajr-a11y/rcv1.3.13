    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if isinstance(func, ast.Attribute):
            if (
                ...
                if name:
                    self.env_calls.append({"var": name, "file": self.path, "line": node.lineno})
        ...
        if func_name in self.resource_names:
            ...
            if path_arg:
                self.asset_calls.append(
                    {"api": "resource_path", "target": path_arg, "file": self.path, "line": node.lineno}
                )
        elif func_name in self.open_names:
            path_arg = self._extract_str_arg(node)
            if path_arg:
                self.asset_calls.append(
                    {"api": "open", "target": path_arg, "file": self.path, "line": node.lineno}
                )
        elif isinstance(func, ast.Name) and func.id in self.path_names:
            path_arg = self._extract_str_arg(node)
            if path_arg:
                self.asset_calls.append(
                    {"api": "Path", "target": path_arg, "file": self.path, "line": node.lineno}
                )
...
def scan_env_and_assets(py_files: Sequence[Path]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for path in py_files:
        try:
            ...
            syntax_errors.append(
                {"file": str(path), "line": exc.lineno, "offset": exc.offset, "msg": exc.msg}
            )
            continue
...
def resolve_asset_candidates(target: str, src: Path) -> Tuple[bool, Optional[Path], List[Path]]:
...
def evaluate_asset_references(asset_calls: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    evaluated: List[Dict[str, Any]] = []
    for ref in asset_calls:
        exists, found_path, candidates = resolve_asset_candidates(ref["target"], ref["file"])
        evaluated.append(
            {
                "api": ref["api"],
                "target": ref["target"],
...
def extract_trace_location(tb: str) -> Tuple[Optional[str], Optional[int], Optional[str]]:
...
def compare_latest_zip() -> Dict[str, Any]:
    ...
    missing_in_workspace = sorted([e for e in normalized_zip if e and e not in workspace_files])
    missing_in_zip = sorted([e for e in workspace_files if e and e not in normalized_zip])
...
def find_pyinstaller_artifacts(py_files: Sequence[Path], zip_info: Dict[str, Any]) -> Dict[str, Any]:
    spec_files = [p for p in ROOT.rglob("*.spec") if p.is_file() and not should_ignore(p)]
    refs: List[str] = []
    for path in py_files:
        try:
            text = path.read_text(encoding="utf-8", errors="ignore")
    ...
    notes: List[str] = []
    if not spec_files:
        notes.append("Nenhum arquivo .spec encontrado; revisar configuracao do PyInstaller manualmente.")
    if not refs:
...
def render_markdown(data: Dict[str, Any]) -> str:
    ...
    def format_command_block(cmd: Dict[str, Any]) -> str:
        ...
        body.append("  - Retorno: nao executado" if rc is None else f"  - Retorno: {rc}")
        if cmd.get("error"):
    ...
    if not data["env_usage"]:
        ...
        lines.append("| Variavel | Definida em `.env` | Definida no ambiente | Onde e usada |")
        lines.append("| --- | --- | --- | --- |")
        for var in data["env_usage"]:
            where = "<br>".join(f"{loc['file']}:{loc['line']}" for loc in var["locations"])
            lines.append(
    ...
    if not data["diagnostic_scripts"]:
        lines.append("- Nenhum script de smoke executado (use --skip-smoke para manter skip explicito).")
        lines.append("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Varredura estatica + smoke do projeto.")
    parser.add_argument("--write-md", default=DEFAULT_REPORT, help="Caminho do markdown de saida.")
    parser.add_argument("--json", help="Opcional: caminho para salvar relatorio em JSON.")
    parser.add_argument(
        "--skip-smoke",
        action="store_true",
...
def main() -> int:
    ...
    if cmd_pip_version.returncode is None and cmd_pip_version.error:
        summary["warnings"].append("pip --version nao encontrado; usando python -m pip --version.")
    ...
    if any(ROOT.glob("requirements*.txt")) or (ROOT / "pyproject.toml").exists():
        ...
        summary["warnings"].append("Nenhum arquivo de requisitos encontrado; pip check nao executado.")
    ...
    for var in env_usage:
        if not var["defined_in_env_file"] and not var["defined_in_os_env"]:
            summary["warnings"].append(f"Variavel {var['name']} nao definida em .env nem no ambiente.")
    ...
    if not args.skip_smoke:
        ...
        if smoke_path.exists():
            ...
            if res.returncode not in (0,):
                summary["errors"].append("scripts/smoke_test.py retornou codigo nao-zero.")

        try:
            import tkinter as tk  # noqa: F401
            root = tk.Tk(); root.update(); root.destroy()
        except Exception as exc:  # pragma: no cover
            tk_result = {"ok": False, "error": f"{type(exc).__name__}: {exc}", "traceback": traceback.format_exc()}
    ...
    if files_browser.exists():
        ...
        if not ok:
            summary["errors"].append("ui/files_browser.py nao expoe open_files_browser.")
        special_checks.append(
            {"label": "ui/files_browser.open_files_browser", "ok": ok, "detail": f"(linha {line_fb})" if line_fb else ""}
        )
    else:
    ...
    if supabase_client.exists():
        ...
        if not ok_zip:
            summary["errors"].append("infra/supabase_client.py sem funcao baixar_pasta_zip.")
        if not ok_exc:
            summary["errors"].append("infra/supabase_client.py sem classe DownloadCancelledError.")
        special_checks.append({"label": "infra/supabase_client.baixar_pasta_zip", "ok": ok_zip, "detail": f"(linha {line_zip})" if line_zip else ""})
        special_checks.append({"label": "infra/supabase_client.DownloadCancelledError", "ok": ok_exc, "detail": f"(linha {line_exc})" if line_exc else ""})
    else:
    ...
    report = {
        ...
        "summary": summary,
        "commands": commands,
        "pip_check": pip_check.to_dict() if isinstance(pip_check, CommandResult) else pip_check,
